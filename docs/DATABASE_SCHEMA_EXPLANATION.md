...
Real-data dashboard tables also support browser-side pagination plus browser-generated exports. Browser PDFs share an XMETA Pay presentation layer with the canonical logo, report metadata, repeated headings, alternating rows, and page footers. The company School admin accounts and pending Admin registrations screens additionally produce branded Excel workbooks from the same filtered `users` and `admin_profiles` display rows; the workbooks use frozen/filterable headings and exclude account-action controls. Other table screens retain CSV/PDF export. Tuition report and Collection log add school, selected-year, filter, count, and financial-total context; Parent Fee summary keeps its nested tuition-term rows inside the same branded layout. These exports use rows already loaded for the authorized screen after search/filter controls are applied, so they do not require extra database tables or change authoritative records. Protected Reports-page server PDFs remain a separate export path.
