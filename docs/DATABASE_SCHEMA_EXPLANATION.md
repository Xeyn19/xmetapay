...
Real-data dashboard tables also support browser-side pagination plus CSV and PDF exports. Browser PDFs share an XMETA Pay presentation layer with the canonical logo, report metadata, repeated headings, alternating rows, and page footers. Tuition report and Collection log add school, selected-year, filter, count, and financial-total context; Parent Fee summary keeps its nested tuition-term rows inside the same branded layout. These exports use the rows already loaded for the signed-in admin or parent after search/filter controls are applied, so they do not require extra database tables or change authoritative accounting data. Protected Reports-page server PDFs remain a separate export path.
