...
The public XMETA EDU entry page uses a shared dark charcoal, orange, and blue branded shell for school-admin and parent/guardian portal choices. The same responsive shell covers role login, registration, and password recovery. Parent registration is invite-only: a school administrator issues a single-use invitation for one student and recorded guardian email, then the guardian verifies a separate email OTP before creating or authenticating the Parent account. Company super admins use the unlisted direct route `/login`; the public landing page does not link to it, and no public company registration route exists. The unlisted URL is only a navigation choice—company authentication and protected-route checks remain authoritative.
...
| --- | --- | --- | --- |
| `school_administrator` | School owner and setup manager | Set up school records, manage school years/sections and school email templates, manage Parent invitations and guardian access, review promote/repeat rollover placements, activate the next school year, manage students, view finance pages, view reports | Company super admin controls |
...
12. School administrators invite the recorded guardian from the exact Student Profile. Parents claim one student through the emailed single-use code and email OTP; every additional child requires another same-school invitation.
...
Guardian access is reversible and school-owned. Only the `school_administrator` can issue, resend, or revoke an unused invitation, or revoke/restore a claimed guardian link with a reason. Registrar and finance roles cannot perform these actions. Revocation hides that student and blocks new Parent operations immediately while retaining the relationship row and all historical financial and audit records.
