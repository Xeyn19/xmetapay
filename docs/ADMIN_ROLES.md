...
The two company account screens replace their plain CSV controls with branded Excel workbooks. Excel files use the same filtered rows and contact columns as PDF, add the XMETA Pay logo and report context, freeze and filter the data headings, and exclude account-action controls. Other admin and parent table CSV exports remain unchanged.
