...
The public XMETA EDU entry page uses a shared dark charcoal, orange, and blue branded shell for school-admin and parent/guardian portal choices. The same responsive shell covers role login, registration, and password recovery. Parent registration lists active schools and requires one immutable school assignment before same-school student references can be linked. Company super admins use the unlisted direct route `/login`; the public landing page does not link to it, and no public company registration route exists. The unlisted URL is only a navigation choice—company authentication and protected-route checks remain authoritative.
...
School administrators and registrars can record optional guardian emails during single or batch student enrollment and correct or cancel pending email assignments on Enrolled students. Only the school administrator can approve a pending Parent account. Enrollment email matching never activates a pending or rejected account; active same-school accounts can be connected as soon as the school records the email.
...
12. Parents choose one active school during registration and may submit up to ten student references. Signup requires at least one same-school reference match or a pending assignment for the registered email; otherwise the form shows a reference alert without creating an account. New accounts remain `pending` without a session or guardian link. Only that school's administrator can approve after rechecking the same-school reference or recorded-email match, reject, or reopen a rejected request. Approval links all matching references and recorded assignments; unmatched references remain saved. This review is school-wide and does not depend on the selected school year. Registrars see the approved contact directory but cannot review requests.
...
Parents can top up one or as many as 20 same-school linked student wallets in one reviewed batch. Each student keeps an independent active-year payment, receipt, and wallet transaction; the parent-owned batch provides atomic processing, retry safety, and a combined result page without merging financial records. An inactive assigned school retains historical reads but blocks new links and financial writes.

Existing parent profiles are migrated conservatively. Only unambiguous single-school evidence is backfilled; unresolved or legacy multi-school accounts keep all database records but receive an account-review screen instead of Parent portal data or actions.
