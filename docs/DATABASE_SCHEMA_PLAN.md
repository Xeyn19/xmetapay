...
Reports are generated from query views over payments, fee assignments, wallets, store transactions, and reminder history instead of storing separate report rows. CSV and PDF report exports are implemented for monthly revenue, tuition collections, outstanding balances, and wallet/store activity. Tuition collection exports use fee or term allocations and exclude wallet-only payments. Real-data super-admin, admin, and parent table screens paginate loaded rows on screen and export all filtered rows as CSV or as a shared branded browser PDF. The company School admin accounts export reads the already authorized `users`/`admin_profiles` result and adds filter and status-count context; Admin registrations reuses its pending-only subset and excludes review controls. The browser renderer adds the XMETA Pay logo, report metadata, repeated headings, alternating rows, and page footers; Tuition report and Collection log also include school, selected year, filters, counts, and filtered totals, while Parent Fee summary retains nested term rows. This presentation layer adds no report storage or accounting fields, and the protected Reports-page server PDFs remain unchanged. Scheduled delivery and notification-based report alerts can be added later.
