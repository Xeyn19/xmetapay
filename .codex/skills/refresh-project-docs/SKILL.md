...
description: Refresh XMETA Pay README, project documentation, and browser visual plans from the current implementation. Use when the user invokes `$refresh-project-docs`, asks to update `README.md` or all files in `docs/`, asks to refresh `public/PROJECT_FLOWCHARTS_VISUAL.html` or `public/DATABASE_SCHEMA_VISUAL_PLAN.html`, or wants the README, docs, flowcharts, schema plan, checklist, and visual HTML files synchronized with recent project changes. Always use the `feature-branch-context` workflow before editing.
---

# Refresh Project Docs

Update the XMETA Pay README and documentation set so they reflect the current project state while keeping public documentation safe and useful.
...
If the checker reports a missing root README or missing/extra direct Markdown files in `docs/`, stop and update this skill's target list or ask the user before refreshing docs.
...
4. Gather current project facts before writing:
   ...
   - Inspect `README.md`, existing docs, and visual HTML structure before editing.
...
5. Update `README.md`, all required Markdown docs, and both visual HTML files together so they agree with each other.
...
- Keep `README.md` as the concise public project entry point. Align its current capabilities, setup/import guidance, primary routes, documentation links, and last-updated footer with verified implementation without copying the detailed docs into it.
...
```powershell
node .codex\skills\refresh-project-docs\scripts\check-doc-targets.mjs
git diff -- README.md docs public\PROJECT_FLOWCHARTS_VISUAL.html public\DATABASE_SCHEMA_VISUAL_PLAN.html
```
...
Report the branch used, the README, docs, and visual files updated, the key project facts synchronized, any sensitive information intentionally omitted, and the verification commands run.
