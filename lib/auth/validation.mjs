...
export function parseRegisterForm(role, formData) {
  ...
  const schoolId = positiveInteger(value(formData, "schoolId"));
  const studentReferences = studentReferenceValues(formData);
  const studentReference = studentReferences[0] ?? "";

  requireField(errors, "phone", base.phone, "Phone number is required.");
  if (studentReferences.length > 10) {
    errors.studentReferences = "Add no more than 10 student references.";
  } else if (studentReferences.some((reference) => reference.length > 60)) {
    errors.studentReferences = "Each student reference must be 60 characters or fewer.";
  }
  if (!schoolId) {
    errors.schoolId = "Choose your school.";
  }
  if (!relationship) {
  ...
  return finish(errors, {
    ...
    phone: base.phone || null,
    password: base.password,
    profile: { schoolId, studentName: studentReference, studentReference, studentReferences, relationship },
  });
}
