...
export function parseRegisterForm(role, formData) {
  ...
  const studentFirstName = value(formData, "studentFirstName");
  const studentMiddleName = value(formData, "studentMiddleName");
  const studentLastName = value(formData, "studentLastName");
  const studentName = [studentFirstName, studentMiddleName, studentLastName].filter(Boolean).join(" ");
  const studentReference = value(formData, "studentReference");

  requireField(errors, "studentFirstName", studentFirstName, "Student first name is required.");
  requireField(errors, "studentLastName", studentLastName, "Student last name is required.");
  ...
  return finish(errors, {
    ...
    phone: base.phone || null,
    password: base.password,
    profile: { studentName, studentReference, relationship },
  });
}
