...
const schoolSetupModalsPath = "app/admin/_components/school-setup-management-modals.tsx";
const schoolYearStructureFormPath = "app/admin/_components/school-year-structure-form.tsx";
const schoolYearStructurePagePath = "app/admin/(dashboard)/school-setup/years/[yearId]/page.tsx";
const schoolYearRolloverPagePath = "app/admin/(dashboard)/school-setup/rollover/page.tsx";
...
test("school setup uses an overview hub with focused management workflows", () => {
  ...
  assert.equal(existsSync(schoolSetupModalsPath), true);
  assert.equal(existsSync(schoolYearStructureFormPath), true);
  assert.equal(existsSync(schoolYearStructurePagePath), true);
  assert.equal(existsSync(schoolYearRolloverPagePath), true);
  ...
  const actions = readFileSync(schoolSetupActionPath, "utf8");
  const modals = readFileSync(schoolSetupModalsPath, "utf8");
  const structureForm = readFileSync(schoolYearStructureFormPath, "utf8");
  const structurePage = readFileSync(schoolYearStructurePagePath, "utf8");
  const rolloverPage = readFileSync(schoolYearRolloverPagePath, "utf8");
  ...
  assert.match(page, /Rename duplicate year labels before activation/);
  ...
  assert.match(page, /EditSchoolDetailsModal/);
  assert.match(page, /AddSchoolYearModal/);
  assert.doesNotMatch(page, /ManualSchoolSetupForm/);
  assert.doesNotMatch(page, /SchoolYearRolloverForm/);

  assert.match(structurePage, /getAdminSchoolYearStructure\(session\.userId, yearId\)/);
  assert.match(structurePage, /notFound\(\)/);
  assert.match(structurePage, /<SchoolYearStructureForm data=\{data\} \/>/);
  assert.match(structurePage, /Back to School setup/);
  assert.match(structureForm, /saveSchoolYearStructureAction/);
  assert.match(structureForm, /name="gradeSetup"/);
  assert.match(structureForm, /Add grade level/);
  assert.match(structureForm, /Add section/);

  assert.match(rolloverPage, /getAdminSchoolRolloverData\(session\.userId\)/);
  assert.match(rolloverPage, /<SchoolYearRolloverForm data=\{data\} \/>/);
  assert.match(rolloverPage, /Choose years/);
  assert.match(rolloverPage, /Select students/);
  assert.match(rolloverPage, /Review placements/);
  assert.match(actions, /export async function updateSchoolDetailsAction/);
  assert.match(actions, /export async function createSchoolYearAction/);
  assert.match(actions, /export async function updateSchoolYearAction/);
  assert.match(actions, /export async function saveSchoolYearStructureAction/);
  assert.match(actions, /redirect\("\/admin\/school-setup\/rollover"\)/);
...
test("backend checklist tracks completed school setup backend slice", () => {
  ...
  assert.match(checklist, /- \[x\] Add an overview-first School setup hub/);
  assert.match(checklist, /\/admin\/school-setup\/years\/\[yearId\]/);
  assert.match(checklist, /\/admin\/school-setup\/rollover/);
