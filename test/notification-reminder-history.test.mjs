...
test("admin payment reminder action queues and sends real email reminders", () => {
  ...
  assert.match(actions, /st\.student_reference/);
  assert.match(actions, /sfa\.status IN \('open', 'partial'\)/);
  assert.match(actions, /getReminderFeeStatements/);
  assert.match(actions, /FROM tuition_payment_terms/);
  assert.match(actions, /feeStatements\.get\(row\.student_id\)/);
  assert.match(actions, /earliestOfficialDueDate/);
  assert.match(actions, /options\.sendTo === "overdue_tuition"/);
  assert.match(actions, /ft\.category = 'tuition'.*sfa\.due_date < CURRENT_DATE/s);
...
test("nodemailer uses a server-only pooled SMTP transport and safe environment configuration", () => {
  ...
  assert.match(mailer, /export type PaymentReminderFee/);
  assert.match(mailer, /Student reference/);
  assert.match(mailer, /Total outstanding/);
  assert.match(mailer, /Earliest official due date/);
  assert.match(mailer, /Fee details/);
  assert.match(mailer, /Installment schedule/);
  assert.match(mailer, /Term dates are installment schedule information/);
  assert.match(mailer, /Sign in to review and pay/);
  assert.match(mailer, /contact your school finance office/);
  assert.match(mailer, /<meta name="viewport"/);
