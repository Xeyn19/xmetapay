...
test("admin real-data helper reads supported MySQL schema tables with admin school scope", () => {
  ...
  assert.doesNotMatch(helper, /recentFeeAssignments/);
  assert.doesNotMatch(helper, /getRecentFeeAssignments/);
...
test("admin real-data tables use working search filters plus spreadsheet and PDF export controls", () => {
  ...
  assert.match(controls, /export async function exportRowsToExcel/);
